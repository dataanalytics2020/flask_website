...
prefecture_df = pd.read_csv(r'csv\pref_lat_lon.csv')
prefecture_list = list(prefecture_df['pref_name'])
for prefecture in prefecture_list[:16]:#[12:13]
    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    post_line_text(f'{now} {prefecture} {hall_name_text} {target_day.strftime("%Y-%m-%d")} ホールナビサイトから座標取得成功',line_token)
                                except Exception as e:
                                    ...
                                    post_line_text(f'{now} {prefecture} {hall_name_text} {target_day.strftime("%Y-%m-%d")} ホールナビサイトから座標取得失敗',line_token)
